...
a button. The sequencing operator
\begin{verbatim}
(>>>=) :: Event a-> (a-> IO b)-> Event b
\end{verbatim}
attaches an IO action to an event; if we synchronize on this event,
the IO action will be performed after the event occurs. The final
operator turns an IO action into an event:
\begin{verbatim}
always :: IO a-> Event a 
\end{verbatim}
Monads form a monad, with monad composition given by \texttt{>>>=} and
\texttt{sync}. The monad composition corresponds to event sequencing;
if we synchronise on an event composed from simple events, we wait for
each of them in turn. By composing events in this way, complex
...
\texttt{IO} action returned kills the concurrent thread; most of the
time, we will not need this.
...
\subsection{The class \texttt{Destructable}}
...
them from the screen. In particular, calling destroy on the return
...
\end{xcode}
implements a \emph{monitor}, like the synonymous method in the
...
These are essentially Tk's widgets. We have simple widgets which are
explained in short sentences of words with less than three syllables,
menues, text widgets, and canvasses, which require a slightly longer
explanation.
...
What is commonly known as a window is called a \emph{toplevel} in Tk
and \HTk.
