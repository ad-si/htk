...
toItem  gui@(GenGUI _ _ _ _ idref _ _ _ _ _ _ _ _) it@(FolderItem _ ch) =
  do
    id <- newID idref
    intch <- mapM (toItem gui) ch
    intchref <- newRVar intch
    return (IntFolderItem gui id it intchref)
toItem gui@(GenGUI _ _ _ _ idref _ _ _ _ _ _ _ _) it@(LeafItem _) =
  do
    id <- newID idref
...
data GenGUI =
  GenGUI (TreeList Item)                                   -- folders tree
...
getNotepad (GenGUI _ np _ _ _ _ _ _ _ _ _ _ _) = np
...
newGenGUI =
  do
    ...
    let gui = (GenGUI tl np ed win idref displayref posref intstate
...
tlObjectSelected tlselmsgQ posref mobj =
  let addNotepadItem :: Item -> IO ()
      addNotepadItem item@(IntLeafItem (GenGUI _ np _ _ _ _ _ _ _ _ _ _ _)
                                       _ (LeafItem ext)) =
        do
          nm <- get ext
          ioimg <- get ext
          ...
          newNotepadItem item np
                         [position pos, photo img,
                          name (Name {full  = full nm,
                                      short =
                                        \n -> take n (full nm)})]
          done
  in case mobj of
       ...
       Just obj ->
         ...
         in do
              gui@(GenGUI _ _ _ _ _ displayref _ _ _ _ _ _ _) <-
                getGenGUI item
              ...
              synchronize gui
                (do
                   ...
                   initItemPosition posref
                   mapM addNotepadItem (filter isLeaf ch)
...
children (Root mguiref) =
  do
    mgui <- getVar mguiref
    case mgui of
      (Just (GenGUI _ _ _ _ _ _ _ intstate _ _ _ _ _)) ->
        do
...
addItem par@(IntFolderItem (gui@(GenGUI tl np _ _ _ displayref posref _
                                        addmsgQ _ _ _ _))
                           _ _ chref) newitem =
  synchronize gui
    (do
...
addItem (Root mguiref) newitem =
  do
    mgui <- getVar mguiref
    case mgui of
      Just gui@(GenGUI tl _ _ _ _ _ _ intstate addmsgQ _ _ _ _) ->
        synchronize gui
          (do
             items <- getVar intstate
             item <- toItem gui newitem
...
addedItem (GenGUI _ _ _ _ _ _ _ _ addmsgQ _ _ _ _) =
  lift (receive addmsgQ)

focusedItemInTreeList :: GenGUI -> IA (Maybe Item)
focusedItemInTreeList (GenGUI _ _ _ _ _ _ _ _ _ tlfocusmsgQ _ _ _) =
  lift (receive tlfocusmsgQ)

selectedItemInTreeList :: GenGUI -> IA (Maybe Item)
selectedItemInTreeList (GenGUI _ _ _ _ _ _ _ _ _ _ tlselmsgQ _ _) = 
  lift (receive tlselmsgQ)

selectedItemInNotepad :: GenGUI -> IA (Item, Bool)
selectedItemInNotepad (GenGUI _ _ _ _ _ _ _ _ _ _ _ npselmsgQ _) =
  lift (receive npselmsgQ)

droppedOnItemInNotepad :: GenGUI -> IA (Item, [Item])
droppedOnItemInNotepad (GenGUI _ _ _ _ _ _ _ _ _ _ _ _ npdropmsgQ) =
...
toTreeListObjects (it@(IntFolderItem gui id (FolderItem ext _) chref) :
  ...
  do
    ...
    nm <- get ext
    ch <- children it
    let nod = if (any isFolder ch) then Node else Leaf
...
instance Eq GenGUI where
  (GenGUI _ _ _ win1 _ _ _ _ _ _ _ _ _) ==
    (GenGUI _ _ _ win2 _ _ _ _ _ _ _ _ _) = win1 == win2

instance GUIObject GenGUI where
  toGUIObject (GenGUI _ _ _ win _ _ _ _ _ _ _ _ _) = toGUIObject win
  cname _ = "GenGUI"
...
instance ToplevelWindow GenGUI where
  iconify (GenGUI _ _ _ win _ _ _ _ _ _ _ _ _) = iconify win
  deiconify (GenGUI _ _ _ win _ _ _ _ _ _ _ _ _) = deiconify win
  withdraw (GenGUI _ _ _ win _ _ _ _ _ _ _ _ _) = withdraw win
  putWinOnTop (GenGUI _ _ _ win _ _ _ _ _ _ _ _ _) = putWinOnTop win
  putWinAtBottom (GenGUI _ _ _ win _ _ _ _ _ _ _ _ _) = putWinAtBottom win

instance Synchronized GenGUI where
  synchronize (GenGUI _ _ _ win _ _ _ _ _ _ _ _ _) = synchronize win
