...
newNotepadItem val notepad@(Notepad cnv _ _ _ _ _) cnf =
  do
    ...
    item <- return(NotepadItem img txt itemval itemname itemsel smsgQ dmsgQ)
...
instance HasPhoto(NotepadItem a) where
  photo i item@(NotepadItem img _ _ _ _ _ _) = img # photo i >> return item
...
data Notepad a =
  Notepad Canvas (Maybe (ScrollBox Canvas)) (RVar ([NotepadItem a]))
          (RVar ([NotepadItem a])) (RVar (Maybe (NotepadItem a)))
          (RVar (Maybe (NotepadItem a, Rectangle, Rectangle))) deriving Eq
...
selectItem notepad@(Notepad cnv _ _ selecteditemsref _ _)
           item@(NotepadItem img _ _ _ _ smsgQ _) =
  do
    Just i <- getPhoto img
    (x,y) <- getSize i
...
selectItemsWithin (x0, y0) (x1, y1)
                  notepad@(Notepad _ _ notepaditemsref selecteditemsref _ _) =
  do
    notepaditems <- getVar notepaditemsref
...
newNotepad scrolled cnf =
  ...
  where click :: Notepad a -> InterActor -> IA ()
                           ...
                           case entereditem of
                             ...
                             Just item@(NotepadItem img _ _ _ _ _ _) ->
                               do
                                 ...
                                 become iact (moving notepad tag x y x y iact))
        ...
        selectingByRectangle :: Notepad a -> Rectangle -> Distance ->
                                ...
                                InterActor -> IA ()
        selectingByRectangle notepad@(Notepad cnv _ _ _ _ _) rect x0 y0 x1 y1
                             iact =
          ...
          let doSet item =
              ...
              inDropZone (NotepadItem img _ _ _ _ _ _) =
                do
                  ...
                  return (if x_it - 30 < x && x_it + 30 > x && y_it - 30 < y &&
                             y_it + 30 > y then True else False)
              ...
              checkDropZones' [] =
                do
                  ...
                  case maybeitem of
                    Just (_, rect1, rect2) ->
                      destroy rect1 >> destroy rect2 >> setVar dropref Nothing
                    Nothing -> done
          in do
               notepaditems <- getVar notepaditemsref
               selecteditems <- getVar selecteditemsref
               (let nonselecteditems =
                      filter (\item -> any ((/=) item) selecteditems)
                             notepaditems
        ...
        moving notepad@(Notepad cnv _ _ selecteditemsref _ dropref) tag
          ...
          (mouseEvent cnv (Button1, Motion) >>>=
             \ ((x, y), _) -> do
                                ...
                                become iact (moving notepad tag rootx rooty x y
                                                    iact))
         +> (mouseEvent cnv (ButtonRelease Nothing) >>>
             (do
                drop <- getVar dropref
...
instance HasSize (Notepad a) where
  width s notepad@(Notepad cnv _ _ _ _ _)  = cnv # width s >> return notepad
  getWidth (Notepad cnv _ _ _ _ _) = getWidth cnv
  height s notepad@(Notepad cnv _ _ _ _ _) = cnv # height s >> return notepad
