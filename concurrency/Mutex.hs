...
releaseMutex (Mutex mvar) = do {
        st <- takeMVar mvar;
        current <- getThreadID;
        case st of
                (Just (holder,n),pnd) | current == holder -> 
                        release' mvar holder n pnd
                _ -> do {putMVar mvar st; raise illegalLockRelease}
} where release' mvar _ 1 [] = putMVar mvar (Nothing,[])
        release' mvar _ 1 ((holder',sem):pnd') = do { 
                putMVar mvar (Just (holder',1),pnd'); release sem}
        release' mvar holder n pnd = putMVar mvar (Just (holder,n-1),pnd)               


tryAcquireMutex :: Mutex -> IO Bool
tryAcquireMutex (Mutex mvar) = do {
        st <- takeMVar mvar;
        current <- getThreadID;
        case st of
                (Nothing,[]) -> do {
                        putMVar mvar (Just (current,1),[]); return True}
                (Just (holder,n),pnd) | current == holder -> do {
                        putMVar mvar (Just (holder,n+1),pnd); return True}
                _ -> do {putMVar mvar st; return False}
} 
