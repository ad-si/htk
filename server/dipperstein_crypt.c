/* crypt3 function which does not require libcrypt.
   This was downloaded by George Russell from
      http://www.nt.fh-koeln.de/fachgebiete/mathe/knospe/crypt3.c
   on 8th December 2004.

   The only change, apart from this comment, is that the function "crypt" has 
   been renamed "dipperstein-crypt", so that the linker doesn't complain
   if an alternative version exists.

   NB.  I know nothing about the copyright situation of this code, and I
   hope the author doesn't mind my using it.
*/
/**************************************************************************
...
*   found on several ftp sites on the internet.  The code has been
*   cleaned and compiles correctly under, gcc, lcc, and Borland's bcc
*   C compilers.  A bug involving the left and right halves of the
*   encrypted data block in the widely published crypt3.c source has been
*   fixed by this version.  All implicit register declarations have been
*   removed, because they generated suboptimal code.  All constant data
*   has been explicitly declared as const and all declarations have been
*   given a minimal scope, because I'm paranoid.
*
...
*              setkey to be envoked with the encryption key before it may
...
*              on to the encryped results.  Same as stored in passwd file.
**************************************************************************/
char *dipperstein_crypt(char *pw, char *salt)
{
    int i, j, temp;
    char c,
    ...
    for(i = 0; i < 2; i++)
    {
        /* store salt at begining of results */
        c = *salt++;
        iobuf[i] = c;
    ...
    /* fromat encrypted block for standard crypt(3) output */
    for(i=0; i < 11; i++)
    {
        c = 0;
        for(j = 0; j < 6; j++)
