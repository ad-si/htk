...
-- parameter may as well be _|_
   ...
   handleClientDisconnect :: (inType,outType,stateType)
      -> User -> stateType -> IO stateType
   -- ^ This function is called at least once when the client disconnects.
   handleClientDisconnect _ _ state = return state
