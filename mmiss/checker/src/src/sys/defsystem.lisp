...
#+(and (or (and allegro-version>= (version>= 4 1))
	   :cmu18)
       (not :logical-pathnames-mk))
(defun logical-pathname-p (thing)
...
;  load-time           ; The file-write-date of the binary/source file loaded.
  ...
  ;; init-function used for dumping systems
  (dump-init-function  nil)
)
  ...
  (let ((pathname
    ...
    (namestring
     (make-pathname :name (pathname-name pathname)
		    ...
		    :device #+(and :CMU (not :cmu17) (not :cmu18)) :absolute
...
(defun component-extension (component type &key local)
  (ecase type
    (:source (or (component-source-extension component)
		 (when (find-language (component-language component))
		   (language-source-extension (find-language (component-language component))))
		 (unless local 
		   (default-source-extension component)))) ; system default
    (:binary (or (component-binary-extension component)
		 (when (find-language (component-language component))
		   (language-binary-extension (find-language (component-language component))))
		 (unless local
...
(defstruct (language (:print-function print-language))
  ...
  binary-load-flag      ; if this flag is true, the system always loads the binaries and never the sources. 
)
...
(defmacro define-language (name &key compiler loader 
				source-extension binary-extension binary-load-flag)
  (let ((language (gensym "LANGUAGE")))
    `(let ((,language (make-language :name ,name 
				     :compiler ,compiler
				     :loader ,loader
				     :binary-load-flag ,binary-load-flag
...
(define-language :lisp
  :compiler 
  #'compile-file 
  ;; #+CMU #'(lambda (filename &rest args) (apply 'compile-file (cons filename (append args (list :block-compile T)))))
  :loader #'load
...
(defun compile-file-operation (component force)
  ...
  (let ((must-compile
	...
	(binary-pathname (directory-namestring (component-full-pathname component :binary))))

    ;; Creating pathname for the binary if necessary...

    (unless (probe-file binary-pathname)
      (warn "Creating full binary pathname ~A" binary-pathname)
      (run-unix-program "mkdir" 
			#+CMU (list "-p" binary-pathname)
			#-CMU (concatenate 'string "-p " binary-pathname)
			))
  ...
  (let* ((binary-pname (component-full-pathname component :binary))
	 ...
	 (check-for-new-source 
	  ...
	  (find force '(:new-source :new-source-and-dependents :new-source-all)
		:test #'eq))
	 (binary-load-flag (and (find-language (or (component-language component)
						  :lisp))
			       (language-binary-load-flag (find-language (or (component-language component)
									     :lisp)))))
	 ...
	 (compile-and-load
	  (and needs-compilation (or load-binary check-for-new-source)
	       (or binary-load-flag (compile-and-load-source-if-no-binary component)))))
    ...
    ;; (format t "compile-and-load = ~A~%load-binary = ~A~%Binary-exists = ~A" compile-and-load load-binary binary-exists)
    (when (or load-source load-binary compile-and-load)
      (cond (compile-and-load
	     ...
	     ;; (format t "~%Loading Binary ~A" component)
	     (with-tell-user ("Loading binary"   component :binary)
		   ...
		   (progn
		     ...
		     (setf (component-load-time component)
			   (file-write-date binary-pname)))))
	     T)
	    ((and source-exists (not binary-load-flag)
