	      ...
	      then
		   (when (or (not kind2) (not (all-xml-whitespace-p val)))
		     (if* (not kind2) then
			     ...
			     (xml-error "unrecognized content '~A'"
		    ...
		    elseif (eq kind :start-tag)
		      then (push (list val) pending)
			   ...
			   (when (iostruct-entity-bufs tokenbuf)
			     (push (if (symbolp val) val (first val)) entity-open-tags))
			   (setf state state-element-contents)
		      else (xml-error "encountered token at illegal syntax position: '~A' following: '~s'"
					(string kind) (first guts)))
	      else
		   (print (list val kind kind2))
				...
				then 
				     (if* (iostruct-entity-bufs tokenbuf) then
					     (when (not (eq (first entity-open-tags) val))
					       (xml-error "~A element closed in entity that did not open it"
							  (string val)))
					     (setf entity-open-tags (rest entity-open-tags))
					else
					     (when (eq (first entity-open-tags) val)
					       (xml-error "~A element closed outside of entity that did not open it"
							  (string val))))
				     (if* (= (length pending) 1)
					then
	   ...
	   (if* (eq kind :pcdata)
	    ...
	    elseif (eq kind :pi)
	      then (push val guts)
		   ...
		   (xml-error "encountered token at illegal syntax position: '~A' following: '~s'"
			      (string kind) (first guts))
		   ))
	  (t
...
(defun next-token (tokenbuf external-callback attlist-data)
  ...
  (macrolet ((add-to-entity-buf (entity-symbol p-value)
    ...
    (let ((state state-pcdata)
	  ...
	  (ns-token)
	  (ch))
      (declare (type (unsigned-byte 16) char-code)
	       (fixnum special-tag-count))
	...
	(case state
	  ...
	  (if* (eq ch #\<)
	   ...
	   elseif (not (xml-char-p ch)) then
		  (xml-error "Illegal character: ~A detected in input" (string ch))
	     else
		  (add-to-coll coll ch)
		  #+ignore
		  (if* (not (eq ch #\return))
	  ...
	  (#.state-pcdata8
	   (if* (eq #\> ch) then
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "content cannot contain ']]>':'~A'" (compute-coll-string coll))
	    elseif (eq #\] ch) then
		   (add-to-coll coll #\])
	      else (setf state state-pcdata)
	    ...
	    elseif (eq #\; ch)
	      then (let ((entity-symbol (compute-tag entity)))
		     ...
		     (if* (not (eq entity-source (first (iostruct-entity-bufs tokenbuf)))) then
			     (xml-error
				"~A reference cannot be constructed from entity reference/character data sequence"
		     ...
		     (if* (string= (symbol-name entity-symbol) "amp") then (add-to-coll coll #\&)
		      ...
		      elseif (string= (symbol-name entity-symbol) "quot") then (add-to-coll coll #\")
			       ...
			       (if* (and (iostruct-do-entity tokenbuf)
					...
					elseif p-value then
						 ...
						 (if* entity-stream then
							 ...
							 (xml-error "Reference to unparsed entity ~A"
								    (string entity-symbol))
							 ))
					       )
				elseif (or (not (iostruct-seen-any-dtd tokenbuf))
				  ...
				  then
				       (xml-error "~A must have entity declaration before being referenced"
						  (string entity-symbol))
				       ))
			     ))
	   ...
	   (let ((code (char-code ch)))
		...
		then (if* (not (eq entity-source (first (iostruct-entity-bufs tokenbuf)))) then
		     ...
		     (when (not (xml-char-p (code-char char-code)))
			   (xml-error "Character reference: ~S (decimal) is not valid XML input character"
				      char-code))
		...
		then (setf char-code (+ 10 (* char-code 16) (- code (char-code #\a))))
		     ...
		     (xml-error "illegal hexidecimal character reference code, starting at: '~A', calculated char code: ~S"
				(compute-coll-string coll) char-code)
		     )))
	   ...
	   (if* (xml-name-start-char-p ch)
	      then (setf state state-readtag-end2)
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "illegal end tag name, starting at: '</~A'"
				(compute-coll-string coll))
		   ))
	    ...
	    elseif (xml-space-p ch) then (setf state state-readtag-end3)
		   ...
		   (setq tag-to-return (compute-tag coll *package*
		     ...
		     (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error "illegal end tag name, starting at: '</~A~A'"
				  tmp (compute-coll-string coll)))
		   ))
	   ...
	   (if* (xml-space-p ch) then nil
	    elseif (eq #\> ch) then (return)
		     ...
		     (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error "illegal end tag name, starting at: '~A' end tag name: ~A"
				(compute-coll-string coll) tmp))
		   ))
	    ...
	    elseif (xml-name-start-char-p ch)
	      then (setf state state-readtag)
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "illegal character following '<', starting at '~A'"
			      (compute-coll-string coll))
		   ))
	   ...
	   (if* (not (eq (elt "CDATA[" special-tag-count) ch))
	      then (clear-coll coll)
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "illegal token following '<![', starting at '<![~A~A'"
	  ...
	  (#.state-readtag-!-conditional5
	   (if* (eq #\] ch)
	    ...
	    elseif (not (xml-char-p ch)) then
		  (xml-error "Illegal character: ~A detected in CDATA input"
			     (string ch))
	  ...
	  (#.state-readtag-!-readcomment
	   (if* (eq #\- ch)
	    ...
	    elseif (not (xml-char-p ch)) then
		   (xml-error "Illegal character: ~A detected in input"
			      (string ch))
	      else (add-to-coll coll ch)))
	  ...
	  (#.state-readtag-end-bracket
	   (if* (eq #\> ch)
	      then (if* (not (eq entity-source (first (iostruct-entity-bufs tokenbuf)))) then
			   (xml-error
			    "~A tag cannot be constructed from entity reference/character data sequence"
		   ...
		   (if* (xml-space-p ch) then
		    ...
		    elseif (eq #\/ ch) then 
			   ...
			   (setf state state-readtag3)
			     ...
			     (if* (null ch)
				then (return)))
			   (xml-error "illegal token name, starting at '~A'"
				      (compute-coll-string coll))
			   )))
	   ...
	   (if* (xml-space-p ch) then nil
	    ...
	    elseif (xml-name-start-char-p ch) then 
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "illegal token, starting at '~A' following element token start: "
			      (compute-coll-string coll) (string tag-to-return))
		   ))
	    ...
	    elseif (xml-space-p ch) then
		   ...
		   (setf state state-readtag12)
		     ...
		     (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error "looking for attribute '=', found: '~A' following attribute name: ~A"
				(compute-coll-string coll) tmp))
		   ))
	   ...
	   (if* (xml-space-p ch) then nil
	    elseif (eq #\= ch) then (setf state state-readtag5)
		 ...
		 (dotimes (i 15)
		   ...
		   (if* (null ch)
		      then (return)))
		   (xml-error "looking for attribute '=', found: '~A' following attribute name: " 
			      (compute-coll-string coll) (string attrib-name))))
	   ...
	   (if* (or (eq ch #\")
	      ...
	      then (setq value-delim ch)
		   (let* ((tag-defaults (assoc tag-to-return attlist-data))
			  (this-attrib (assoc attrib-name (cdr tag-defaults))))
	    ...
	    elseif (xml-space-p ch) then nil
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "attribute value not delimited by ' or \" : '~A' following attribute: "
			      (compute-coll-string coll) (string attrib-name))
		   ))
	   ...
	   (let ((from-entity (and attrib-value-tokenbuf
			 ...
			 (if* package then (setf package (rest package))
				 ...
				 (setf package
				   (let ((i 0) new-package)
				     (declare (fixnum i))
				     (loop
				       (let* ((candidate (format nil "net.xml.namespace.~s" i))
		...
		then (add-to-coll coll ch)
		     ...
		     (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error "attribute value cannot contain '<': '~A' following attribute: "
				(compute-coll-string coll) (string attrib-name)))
	     (setf last-ch ch)))
	    ...
	    elseif (xml-name-start-char-p ch)
	      then (setf state state-readtag9)
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "attribute value contains illegal reference name: '&~A' in attribute value for: "
			      (compute-coll-string coll) (string attrib-name))
		   ))
	    ...
	    elseif (<= (char-code #\0) (char-code ch) (char-code #\9))
	      then (setf state state-readtag11)
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "attribute value contains illegal character reference code: '~A' in attribute value for: ~A"
			      (compute-coll-string coll) (string attrib-name))
		   ))
	  ...
	  (#.state-readtag10
	   (let ((code (char-code ch)))
	     (if* (eq #\; ch)
		then (if* (not (eq entity-source (first (iostruct-entity-bufs tokenbuf)))) then
			     (xml-error "~A reference cannot be constructed from entity reference/character data sequence"
					(string (code-char char-code)))
			else
			     (setf entity-source nil))
		     (add-to-coll coll (code-char char-code))
		...
		then (setf char-code (+ 10 (* char-code 16) (- code (char-code #\a))))
		     ...
		     (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error "attribute value contains illegal hexidecimal character reference code: '~A' in attribute value for: ~A"
				(compute-coll-string coll) (string attrib-name))
		     )))
	   ...
	   (let ((code (char-code ch)))
		...
		then (setf char-code (+ (* char-code 10) (- code (char-code #\0))))
		     ...
		     (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error "attribute value contains illegal decimal character reference code: '~A' in attribute value for: ~A"
				(compute-coll-string coll) (string attrib-name))
		     )))
	   ...
	   (if* (xml-name-char-p ch)
	    ...
	    elseif (eq #\; ch)
	      then (let ((entity-symbol (compute-tag entity)))
		     (clear-coll entity)
		     (if* (not (eq entity-source (first (iostruct-entity-bufs tokenbuf)))) then
			     (xml-error "~S reference cannot be constructed from entity reference/character data sequence"
					(string entity-symbol))
			else
			     (setf entity-source nil))
		     (if* (string= (symbol-name entity-symbol) "amp") then (add-to-coll coll #\&)
		   ...
		   (setq state state-readtag6)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "attribute value contains illegal reference name: '&' in attribute value for: ~A"
			      (compute-coll-string coll) (string attrib-name))
		   ))
	  ...
	  (#.state-readtag3
	   (if* (eq #\> ch) then (return)
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "expected '>' found '~A' in element: " 
			      (compute-coll-string coll) (string tag-to-return))
		   ))
	   ...
	   (if* (xml-name-char-p ch) ;; starting char already passed more restrictive test
	      then
		   ...
		   (when (not (xml-space-p ch))
		     (xml-error "expecting whitespace following: '<!~A' ; got: '~A'"
	  ...
	  (#.state-pre-!-contents
	   (if* (xml-space-p ch)
	      ...
	      ;; no test for this...
	      then (xml-error "illegal character '~A following <!"
			      (string ch) (string tag-to-return))
	    elseif (eq #\> ch)
	      then (return)
	   ...
	   (if* (xml-name-char-p ch)
	    ...
	    elseif (and (xml-space-p ch) (eq tag-to-return :DOCTYPE))
	      ...
	      then (push (compute-tag coll) contents-to-return)
		   ...
		   ;; XXX tag?
	      else (xml-error "illegal name: '~A' in <! tag: "
			      (string tag-to-return))
	      ))
	   ...
	   (if* (xml-name-char-p ch) ;; starting char already passed more restrictive test
	      then
		   ...
		   (let ((token (compute-tag coll)))
			...
			else (xml-error "expected 'SYSTEM' or 'PUBLIC' got '~A' in <! tag: ~A"
					(string (first contents-to-return))
					(string tag-to-return)
	  ...
	  (#.state-!-doctype-public
	   (if* (xml-space-p ch) then nil
	    ...
	    elseif (eq #\' ch) then (setf state state-!-doctype-public3)
	      else (xml-error "expected quote or double-quote got: '~A' in <! tag: ~A ~A ~A"
		      (string ch)
		      (string tag-to-return)
		      (string (second contents-to-return))
		      (string (first contents-to-return)))
	  ...
	  (#.state-!-doctype-public2
	   (if* (eq #\" ch) then (push (compute-coll-string coll) 
	    ...
	    elseif (pub-id-char-p ch) then (add-to-coll coll ch)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "illegal character in DOCTYPE PUBLIC string: '~A'"
			      (compute-coll-string coll))
		   ))
	    ...
	    elseif (eq #\[ ch)
	      then (setf state state-begin-dtd)
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "illegal char in DOCTYPE token: '~A'"
			      (compute-coll-string coll))
		   ))
	   ...
	   (if* (xml-space-p ch) then nil
	    elseif (eq #\> ch) then (return)
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "illegal char in DOCTYPE token following dtd: '~A'"
			      (compute-coll-string coll))
		   ))
	  ...
	  (#.state-!-doctype
	   ...
	   (if* (xml-space-p ch) then nil
	    ...
	    elseif (eq #\[ ch)
	      then (setf state state-begin-dtd)
	      else (xml-error "illegal character: '~A' in <! tag: ~A ~A" 
			      (string ch)
			      (string tag-to-return)
			      (string (first contents-to-return)))
		   ))
	    ...
	    elseif (eq ch empty-delim)
	      then (setf state state-noattributename)
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "expected space or tag end before: '~A'"
			      (compute-coll-string coll))))
	    ...
	    elseif (xml-name-start-char-p ch)
	      then
		   ...
		   (dotimes (i 15)
		     ...
		     (if* (null ch)
			then (return)))
		   (xml-error "illegal char in <?xml token: '~A'"
			      (compute-coll-string coll))
		   ))
	    ...
	    elseif (xml-space-p ch) then
		   ...
		   (when (not (eq #\= ch))
		     (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error	"illegal char in <?xml attribute token: '~A'"
				(compute-coll-string coll)))
		   (setq attrib-name (compute-tag coll))
		   (clear-coll coll)
	  ...
	  (#.state-attribstartvalue
	   ...
	   (if* (or (eq ch #\")
	    ...
	    elseif (xml-space-p ch) then nil
		   ...
		   (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error	"expected ' or \" before  <?xml attribute token value: '~A'"
				(compute-coll-string coll))))
	     ...
	     elseif (and (xml-char-p ch) (not (eq #\< ch)))
	       then (add-to-coll coll ch)
		    ...
		    (dotimes (i 15)
		       ...
		       (if* (null ch)
			  then (return)))
		     (xml-error	"illegal character in attribute token value: '~A'"
				(compute-coll-string coll))))
	   ...
	   (#.state-noattributename
	    (if* (eq #\> ch)
	       ...
	       (xml-error "expected '>' found: '~A' in <?xml token"
			  (string ch))))
...
(defun parse-default-value (value-list tokenbuf external-callback)
  (let (value-string)
    ...
    (let ((tmp-result (parse-xml
		      (concatenate 'string "<item x='" value-string "'/>")
