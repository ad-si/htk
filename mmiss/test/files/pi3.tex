...
\Declare{sieve-finite}{Sieb des Erathostenes}
\Declare{def:striktheit}{Funktion \texttt{f} ist \emph{strikt}}
\Declare{ref:fib-stream}{Damit ergibt sich:}
\Declare{expr-grammar-alt}{Grammatik:}
\Declare{ref:draw}{Zeichenfunktion}
\Declare{ref:st-monad}{Ausführlicher}
\Declare{pexpr-alt}{Parsierung von \texttt{Expr}}
...
\begin{List}[ListType=itemize]
\ListItem \Def{sieve-finite}{Sieb des Erathostenes}
  \begin{List}[ListType=itemize]
...
% `)' muß in den LinkText, sonst Leerraum davor! - CK
   (vgl. \Link[LinkText={alte Version)}]{sieve-finite}
...
\end{xcode}
\end{ProgramFragment}
\ListItem \Declare{ref:fib-stream} \pause{} 
...
\end{verbatim}
  \begin{List}[ListType=itemize]
    \ListItem \Link[LinkText={(vergleiche alt)}]{expr-grammar-alt}
  \end{List}
...
\end{ProgramFragment}
  \begin{List}[ListType=itemize]
  \ListItem \Link[LinkText={(vergleiche alt)}]{pexpr-alt}
  \end{List}
...
\begin{Section}[Title={Eine Geometrie-Bücherei}, Label={section10_2}_geo_lib]
...
\ListItem Damit \texttt{Robot} als Instanz von \texttt{Monad} (vergleiche
  \Link[LinkText={\texttt{ST}):}]{ref:st-monad}
